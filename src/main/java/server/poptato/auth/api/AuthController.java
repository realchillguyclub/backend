...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<LoginResponseDto>> login(
        ...
        LoginResponseDto response = authService.login(loginRequestDto, clientIp, userAgent);
     ...
     * Token Rotation이 적용되어 기존 토큰은 ROTATED 상태로 변경됩니다.
    ...
    public ResponseEntity<ApiResponse<TokenPair>> refresh(
        ...
        String clientIp = clientInfoExtractor.extractClientIp(request);
        String userAgent = clientInfoExtractor.extractUserAgent(request);
        TokenPair response = authService.refresh(reissueTokenRequestDto, clientIp, userAgent);
        return ApiResponse.onSuccess(SuccessStatus._OK, response);
    }
}
